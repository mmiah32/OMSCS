...
def compute_portvals(
    ...
    ledger = pd.DataFrame(0, index=prices.index, columns= [symbol, 'Cash'])

    for index, row in trades.iterrows():
        if index in prices.index:
            price = prices.loc[index, symbol]

            if trades['trade'][index] > 0:
                new_price = price * (1.0 + impact)
                ledger.loc[index, symbol] = trades.loc[index, 'trade']
                ledger.loc[index, 'Cash'] += -(trades.loc[index, 'trade'] * new_price) - commission

            elif trades['trade'][index] < 0:
                new_price = price * (1.0 - impact)
                ledger.loc[index, symbol] = trades.loc[index, 'trade']
                ledger.loc[index, 'Cash'] += -(trades.loc[index, 'trade'] * new_price) - commission
